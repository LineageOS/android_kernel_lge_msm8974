...
int msm_comm_qbuf(struct vb2_buffer *vb)
{
	...
	inst = q->drv_priv;
	if (!inst || !vb) {
		dprintk(VIDC_ERR, "Invalid input: %pK, %p\n", inst, vb);
		return -EINVAL;
	}
	core = inst->core;
	if (!core) {
		dprintk(VIDC_ERR,
			"Invalid input: %pK, %p, %p\n", inst, core, vb);
		return -EINVAL;
	}
	hdev = core->device;
	if (!hdev) {
...
int msm_comm_flush(struct msm_vidc_inst *inst, u32 flags)
{
	...
	if (inst->state == MSM_VIDC_CORE_INVALID ||
			...
			core->state == VIDC_CORE_INVALID) {
		dprintk(VIDC_ERR,
				"Core %pK and inst %p are in bad state\n",
					core, inst);
			...
			int *domain_num, int *partition_num)
{
	if (!inst || !domain_num || !partition_num) {
		dprintk(VIDC_ERR, "%s: invalid params: %pK %p %p\n",
