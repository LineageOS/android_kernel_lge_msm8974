				...
				struct list_head *list, u32 device_addr)
{
	...
	if (!list || !device_addr || !inst) {
		dprintk(VIDC_ERR,
			"Invalid input- list: %pK device_addr: %u inst: %p\n",
...
int map_and_register_buf(struct msm_vidc_inst *inst, struct v4l2_buffer *b)
{
	...
	for (i = 0; i < b->length; ++i) {
		...
		dprintk(VIDC_DBG,
			"%s: [MAP] binfo = %pK, handle[%d] = %p, device_addr = 0x%x, fd = %d, offset = %d, mapped = %d\n",
			...
			struct buffer_info *binfo)
{
	...
	if (!inst || !binfo) {
		dprintk(VIDC_ERR, "%s invalid param: %pK %p\n",
			__func__, inst, binfo);
		return -EINVAL;
	}
	...
	*/
	...
	for (i = 0; i < temp->num_planes; i++) {
		dprintk(VIDC_DBG,
			"%s: [UNMAP] binfo = %pK, handle[%d] = %p, device_addr = 0x%x, fd = %d, offset = %d, mapped = %d\n",
...
int msm_vidc_enum_framesizes(void *instance, struct v4l2_frmsizeenum *fsize)
{
	...
	if (!inst || !fsize) {
		dprintk(VIDC_ERR, "%s: invalid parameter: %pK %p\n",
				__func__, inst, fsize);
		return -EINVAL;
	}
	if (!inst->core)
...
void *msm_vidc_smem_get_client(void *instance)
{
	...
	if (!inst || !inst->mem_client) {
		dprintk(VIDC_ERR, "%s: invalid instance or client = %pK %p\n",
