		...
		u32 flags, enum hal_buffer buffer_type)
{
	...
	if (!iova || !buffer_size || !hndl || !smem_client) {
		dprintk(VIDC_ERR, "Invalid params: %pK, %p, %p, %p\n",
	...
	struct ion_handle *hndl, int domain_num, int partition_num, u32 flags)
{
	...
	if (!hndl || !smem_client) {
		dprintk(VIDC_WARN, "Invalid params: %pK, %p\n",
				smem_client, hndl);
		return;
	}
		...
		struct msm_smem *mem, enum hal_buffer buffer_type)
{
	...
	if (IS_ERR_OR_NULL(hndl)) {
		dprintk(VIDC_ERR, "Failed to get handle: %pK, %d, %d, %p\n",
	...
	dprintk(VIDC_DBG,
		"%s: ion_handle = 0x%pK, fd = %d, device_addr = 0x%x, size = %d, kvaddr = 0x%p, buffer_type = %d\n",
	...
	int map_kernel)
{
	...
	mem->size = size;
	dprintk(VIDC_DBG,
		"%s: ion_handle = 0x%pK, device_addr = 0x%x, size = %d, kvaddr = 0x%p, buffer_type = %d\n",
	...
	struct msm_smem *mem, enum smem_cache_ops cache_op)
{
	...
	int msm_cache_ops = 0;
	if (!mem || !client) {
		dprintk(VIDC_ERR, "Invalid params: %pK, %p\n",
			mem, client);
		return -EINVAL;
	}
